...
public class BotEntry2 implements RequestStreamHandler {
	...
	public void handleRequest(InputStream input, OutputStream output, Context context) {
		...
		try {
			...
			String id = lineMessagingClient.getBotInfo().get().getBasicId();
			String replyToken = node.get("events").get(0).get("replyToken").asText();
			String text = node.get("events").get(0).get("message").get("text").asText() + id;
